...
- portal dev server 跑在 3002，vite 代理 `/api`、`/media`、`/ws` 到 `127.0.0.1:7777`。
- M4 已完成：合集连播页（/mix/:id）、视频详情页（/local-video/:id）、扩展音乐推送。
...
| MV3 插件 | `extension/` | ✅ 已支持 POST/LIKE/FAVORITE/WATCH_LATER/COLLECT_FOLDER/COLLECT_MUSIC 推送 |
...
- **音乐 / 合集 / 收藏夹 / 稍后再看** 的后端 fetcher 已接入；插件侧音乐推送已补齐
- **FTS5 全文搜索**：`content_search` 虚拟表 + 触发器已迁移；前端搜索框已添加到视频 Tab 和音乐 Tab
- 前端「音乐」Tab 已接通（MusicShelf 组件，支持搜索、分页、播放）
- 前端「合集」Tab 已接通（合集列表 + `/mix/:id` 连续播放页）

### 4.3 M4（连播 + 视频详情页）✅

- 合集连播（mix 内多视频顺播）→ `/mix/:id`
- 单视频详情页 → `/local-video/:id`
- 详情页与列表间导航切换
...
| 前端「我的」聚合页 | `web/packages/dolike-portal/src/views/my/index.vue` |
...
| 抖音 URL 对照表 | `web/packages/dolike-portal/src/api/urls.ts` |
