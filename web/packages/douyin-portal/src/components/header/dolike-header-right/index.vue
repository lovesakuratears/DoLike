...
const onBound = () => {
  window.dispatchEvent(new CustomEvent('dolike:douyin-bound'))
...
onMounted(() => {
  window.addEventListener('dolike:douyin-bound', onBoundEvent)
  void refreshDouyinAvatar()
})
onBeforeUnmount(() => {
  window.removeEventListener('dolike:douyin-bound', onBoundEvent)
})
