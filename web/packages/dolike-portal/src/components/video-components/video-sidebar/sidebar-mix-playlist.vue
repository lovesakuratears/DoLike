...
// 从第一条开始播放
const playFromFirst = () => {
  if (awemeList.value.length === 0) return
  const first = awemeList.value[0]
  router.replace({
    path: route.path,
    query: {
      ...route.query,
      modal_id: first.aweme_id
    }
  })
}
...
const toggleSequentialMode = () => {
  sequentialMode.value = !sequentialMode.value
}
...
const playNextInSequence = () => {
  ...
  if (currentIndex >= 0 && currentIndex < awemeList.value.length - 1) {
    const next = awemeList.value[currentIndex + 1]
    router.replace({
      path: route.path,
      query: {
        ...route.query,
        modal_id: next.aweme_id
      }
    })
  }
}
  ...
  <div class="sidebar-mix-playlist">
    ...
    <div class="mix-header">
      ...
      <div class="mix-header-right">
        <DyButton type="secondary" theme="solid" class="play-from-first-btn" @click="playFromFirst">
          从第一条播放
        </DyButton>
        <DyButton type="secondary" theme="solid" class="sequential-btn" :class="{ active: sequentialMode }" @click="toggleSequentialMode">
          {{ sequentialMode ? '✓ 顺序播放' : '顺序播放' }}
        </DyButton>
