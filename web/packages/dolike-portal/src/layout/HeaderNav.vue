...
const mode = ref<'enhanced' | 'compatible'>('enhanced')
const parserStatus = ref<'unknown' | 'available' | 'unreachable' | 'unconfigured'>('unknown')
const loadingMode = ref(false)
const openLog = () => { logOpen.value = true }
const openDownload = () => { downloadOpen.value = true }
const loadMode = async () => {
  try {
    const r = await localApi.downloadMode()
    if (r.code === 0) {
      mode.value = r.data.mode
      parserStatus.value = r.data.parserStatus
    }
  } catch {
    // ignore
  }
...
const toggleMode = async () => {
  ...
  try {
    const next = mode.value === 'enhanced' ? 'compatible' : 'enhanced'
    const r = await localApi.setDownloadMode(next)
    if (r.code === 0) {
      mode.value = r.data.mode
      parserStatus.value = r.data.parserStatus
      if (r.data.mode === 'enhanced' && (r.data.parserStatus === 'unconfigured' || r.data.parserStatus === 'unreachable')) {
        ElMessage.warning('增强模式当前不可用，下载会回退兼容模式。建议先切到兼容模式等待解析服务恢复。')
      } else {
        ElMessage.success(mode.value === 'enhanced' ? '已切换为增强模式（无水印）' : '已切换为兼容模式（有水印）')
      }
    }
  } catch (e: any) {
...
<template>
  <div class="header">
    ...
    <DownloadPanel v-model:open="downloadOpen" />
    <LogPanel v-model:open="logOpen" />
  </div>
</template>
